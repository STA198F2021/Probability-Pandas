...
#add better labels for this table and create a table title
...
#please make these nicer tables
...
```{r Simple-Marginal-Relationship8, warning=FALSE, message=FALSE, echo=FALSE}
#madeleine's graph for health access vs checkup
plot1 <- ggplot(cities, aes(x = health_access,
                   y= checkup))+
  geom_point()
```


```{r Simple-Marginal-Relationship9, warning=FALSE, message=FALSE, echo=FALSE} 
 cities1 <- cities %>%
   na.omit(city_size)

plot2 <- ggplot(cities1, aes(x = city_size, 
                   y = checkup_n))+ 
