...
```{r Prediction-enzo show this to prof. herring}
...
ggplot(cities_test1, aes(x = test_prob1, y = checkup)) +
  geom_point(aes(color = region, size = health_access)) +
  labs(title = "Predicted Checkup Prevalence (within a year) vs Collected Data",
       x = "Predicted Checkup Prevalence", y = "Checkup Prevalence")
...
ggplot(cities_test2, aes(x = test_prob2, y = checkup)) +
  geom_point(aes(color = region, size = health_access))
...
rmse(model_cities1, truth = checkup, estimate = test_prob_model)
...
rmse(model_cities2, truth = checkup, estimate = test_prob_model)

#calculate root mean standard error of both models against test data set

rmse(cities_test1, truth = checkup, estimate = test_prob1)
rmse(cities_test2, truth = checkup, estimate = test_prob2)
